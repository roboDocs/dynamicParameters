...
class ParameterModeError(Exception):
    ...
    msg = "A slave parameter's mode cannot be"

    def __init__(self, mode):
        self.mode = mode

    def __str__(self):
        return '%s %s' % (self.msg, repr(self.mode))

class ParameterValueError(Exception):

    msg = "A parameter needs a default value or master."
...
class SingleValueParameter(object):
    ...
    def __init__(self, name, defaultValue=None, limits=None, numType='float', master=None, mode=None):
        self.name = name
        self.master = master
        if (master is None) and (defaultValue is None):
            raise ParameterValueError
            return
        if (master is None) and (defaultValue is not None):
    ...
    def setLimits(self, minMaxValues):
        self.limits = minMaxValues # (minValue, maxValue)
        for slave in self.slaves:
            slave.limits = minMaxValues # (minValue, maxValue)
            slave.value = slave.get()
            slave.update()
